...
# global log stream; log_stream.getvalue() will have the result
log_stream = StringIO()
logging.basicConfig(stream=log_stream, level=logging.INFO)
today = datetime.now().astimezone(ZoneInfo('America/Phoenix'))
today_str = today.strftime('%m/%d/%Y, %H:%M:%S')
logging.info(f'daily pmp_sftp backup begun {today_str}')
    ...
    if time_dif <= timedelta(hours=24):
        ...
        if not file_exists:
            ...
            if remote_file_mtime > drive_file_modified_time:
                logging.info(f'updating {remote_file} on google drive...')
                # sftp -> google drive
                with sftp.file(remote_file_path, 'rb') as remote_file_content:
                    remote_file_content.prefetch()
                    media = MediaIoBaseUpload(remote_file_content, mimetype='application/octet-stream', chunksize=1024*1024, resumable=True)
...
def backup_sftp():
    sftp_host = os.environ.get('pmp_host', 'environment variable is not set')
    sftp_port = os.environ.get('pmp_port', 'environment variable is not set')
    sftp_user = os.environ.get('pmp_user', 'environment variable is not set')
    sftp_password = os.environ.get('pmp_password', 'environment variable is not set')
    remote_path = os.environ.get('pmp_path', 'environment variable is not set')

    drive_folder_id = os.environ.get('pmp_folder', 'environment variable is not set')
...
def hello_pubsub(event, context):
    ...
    today = datetime.now().astimezone(ZoneInfo('America/Phoenix'))
    today_str = today.strftime('%m/%d/%Y, %H:%M:%S')
    logging.info(f'daily pmp_sftp backup complete {today_str}')
